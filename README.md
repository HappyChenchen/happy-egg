...
1. A 右击宠物，选择“添加好友”→“复制宠物号”，把 6 位号码发给 B。
2. B 选择“添加好友”→“输入宠物号…”，发送好友申请。
3. A 在右键菜单的“好友申请”中接受；双方会自动保存好友。
4. 双方都保留好友且 MacPet 在线时，好友列表会显示在线并开放互动。

单击宠物只切换自己的表情；双击宠物会拍一拍当前在线好友。拖动宠物不会触发互动，爱心和庆祝仍可从右键菜单发送。

右键菜单里可以给当前好友发短文字留言或预设贴纸。拍一拍、爱心、庆祝要求好友在线，而留言即使好友离线也能发送——relay 会暂存留言并在对方下次上线时送达。收到的留言会显示在有界多行气泡里，并保存在“消息记录”中标记未读；点击后可在滚动详情中查看完整内容。Mac 与网页端都会先保存本地副本，再确认 relay 删除服务端副本。

宠物号不会在使用一次后失效，也不要求双方同时在线；离线期间收到的申请会在下次启动时出现。选择好友只会切换当前互动对象，App 会记住上次选择；如果只有一个好友，启动时会自动选中。删除好友后需要重新发送申请才能再次互动。右键菜单也允许更换宠物号，更换不会删除已有好友。
...
宠物名字、稳定 ID、设备认证令牌、好友列表和最近的留言保存在本机。Relay 会持久化宠物号映射、认证令牌哈希、服务端确认的好友关系与删除墓碑、未完成的好友申请和未确认留言，只在内存中维护在线连接。实时互动不持久化；好友留言会以明文保存到接收方确认本地落盘或 7 天 TTL 到期，清理周期最多再延迟 1 分钟；每个接收方最多 50 条、全局最多 5000 条。当前版本没有账户系统，不能替代高安全等级的身份认证方案；详细边界见 [SECURITY.md](SECURITY.md)。
